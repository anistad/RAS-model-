...
def run_power(scale):
    ...
    department1_dict = {'number tanks': no_tanks[0]*scale, 'diameter': 8, 'HRT': 40, 'specific density': SD_department1, 'salinity': 0.003, 'recirc degree': 0.985}
    department2_dict = {'number tanks': no_tanks[1]*scale, 'diameter': 8, 'HRT': 40, 'specific density': SD_department2, 'salinity': 0.003, 'recirc degree': 0.985}
    department3_dict = {'number tanks': no_tanks[2]*scale, 'diameter': 14, 'HRT': 40, 'specific density': SD_department3, 'salinity': 0.003, 'recirc degree': 0.985}
...
ax.scatter(24000,	3424.684932, marker = "^",color='black')
ax.scatter(3700, 237.6229055, marker = "^", color="black")
ax.set_ylabel('Average power (kW)', fontsize=16)
ax.set_xlabel('Volume  (m$^3$)', fontsize=16)
ax.legend(frameon=False, fontsize=16)
ax.tick_params(axis='both', which='major', labelsize=16)
fig.tight_layout()

fig.savefig('Model_data_flow_rate_power.eps')
